...
CREATE TABLE Motiv(
			motiv_id int NOT NULL PRIMARY KEY,
			naziv varchar(100)
);
CREATE TABLE Umjetnina_Motiv(
			um_id int NOT NULL,
			m_id int NOT NULL,
			interpretacija varchar(255)
);
...
ALTER TABLE  Umjetnina_Motiv
ADD FOREIGN KEY (um_id)  REFERENCES  Umjetnina(umjetnina_id);

ALTER TABLE  Umjetnina_Motiv
ADD FOREIGN KEY (m_id)  REFERENCES  Motiv(motiv_id);
